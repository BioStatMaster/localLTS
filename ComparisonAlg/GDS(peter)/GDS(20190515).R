...
##### Simulations for GDS ####
GDS_simulation_fun = function(seed, n_real, p_real, d = 1, path = F){
  ...
  A = apply(data, 2, var) #  max(A)/min(A) < 4
  if( 1==1 ){
    result_GDS = GDS_Algorithm(data, scoreName = "SEMSEV", pars = list(regr.pars = list()), check = "checkUntilFirstMinK", output = FALSE, startAt = "randomGraph", graph = graph)
  }else{
    result_GDS = NULL
  }
...
GDS_Algorithm = function(data, scoreName = "SEMSEV", pars = list(regr.pars = list()), check = "checkUntilFirstMinK", output = FALSE,startAt = "emptyGraph", graph = NULL){
  ...
  if( !is.null(graph) ){
    ...
    B[B!=0] =1
    MEC = B + t(B)
    evaluation_result_GSEM = evaluation_fun( B, result_DAG ) 
    evaluation_result_GSEM_MEC = evaluation_fun( dag2cpdagAdj(B), result_MEC)
  }
