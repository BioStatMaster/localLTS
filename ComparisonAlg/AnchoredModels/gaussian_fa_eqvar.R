...
FA_EQVAR =  function(data, alpha=0.001, nleaf, graph=NULL){  
  ...
  if((nleaf==0) & (!is.null(graph))){ ## 0이면 true leaf 또는 true leaf가 작을 때 identifiable하기 위한 minimum leaf 넣어줌
    B = graph
    B[B!=0] =1
  ...
  non_leaf = tail(order(Estar_var, decreasing = TRUE),K)
  ...
  for (i in 1:length(leaf)){
    ...
    for (k in 1:K){
      ...
      cov(cbind(leaf_data,X[,non_leaf]),method = 'pearson')
      pcor(cbind(leaf_data,X[,non_leaf]),method = 'pearson')
      result <- pcor.test(leaf_data, nonleaf_data, X[,non_leaf[-k]], method = "pearson")
  ...
  ####
  print(paste("FA_EQVAR takes: ", Runtime))
  if( !is.null(graph) ){
    B = graph
    B[B!=0] =1
