function B = updateB_SCAD(B, X, posMeanE, posCovE, Lambda, B0)
...
[B,fun,time,iter] = gistIFA(posMeanE,posCovE,X,Lambda,lambda,theta,...
...
function [w,fun,time,iter] = gistIFA(posMeanE,posCovE,X,Lambda,lambda,theta,varargin)

% Generalized Iterative Shrinkage and Thresholding (GIST) for sparse IFA
%
...
grad = loss_mse_grad(w, X, posMeanE, posCovE, Lambda)/n;
fun(1) = loss_mse(w, X, posMeanE, posCovE, Lambda)/n + funRegC(w,d^2,lambda,theta,regtype);
time(1) = 0;
DEBUG = false;
if DEBUG
    numGrad = computeNumericalGradient(@(x)loss_mse_em(x, X, posMeanE, posCovE, Lambda), w);
...
for iter = 1:maxiter
    ...
    for inneriter = 1:maxinneriter
        w = w_old - grad_old/t;
        w(1:d+1:end) = zeros(1,d);
        w = proximalRegC(w_old - grad_old/t, d^2, lambda/t, theta,regtype);
        dw = w - w_old;
        fun(iter+1) = loss_mse(w, X, posMeanE, posCovE, Lambda)/n + funRegC(w,d^2,lambda,theta,regtype);  
