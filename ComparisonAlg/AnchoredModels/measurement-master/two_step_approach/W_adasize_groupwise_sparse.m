function [y, W] = W_adasize_groupwise_sparse(lambda1,lambda2, Mask, WW, x_all)
% ICA with SCAD penalized entries of the de-mixing matrix
% WW: initialization;
% x_all: structure contains all demixing matrices
% lambda1: regularization parameter for the similarity between two B matrices
% lambda2: regularization parameter for the sparsity
...
%     fprintf('.');
    ...
    if ~mod(iter,1000)
        save(strcat('W_',num2str(iter/1000)),'W');
    end
    total_grad = 0;
    for ii = 1:Num_Sub
        y{ii} = W{ii} * x_all{ii};
...
%         dev{ii} = dev{ii} + const * tanh(m*W{ii});
%         
%         grad_new{ii} = y_psi{ii}*x_all{ii}'/T + inv(W{ii}') - lambda/T * dev{ii};
        
        grad_new{ii} = y_psi{ii}*x_all{ii}'/T + inv(W{ii}') - lambda1/T * dev{ii} - lambda2/T * tanh(m*W{ii});
