function [posMeanE, posCovE, posMeanEqi, posCovEqi, posCovEq0] = evaluateE(Y, posq, A, Lambda, ...
...
for q = 1:size(posMeanE,3)
    ...
    posCovEq(:,:,:,q) = repmat(posCovEq0(:,:,q),1,1,N)+repmat(rshPosMeanEV,1,Dim).*...
    repmat(rshPosMeanEH,Dim,1);
    posMeanE(:,:,q) = repmat(posq(:,q)',[Dim 1]) .* posMeanE(:,:,q);
end
for q = 1:numGauss
    indicies = qidMat==q;
    indicies = reshape(indicies', Dim, 1, numGauss^Dim1);
    indicies = repmat(indicies, 1, N, 1);
