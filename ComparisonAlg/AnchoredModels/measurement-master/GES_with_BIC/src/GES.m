...
score = Score_G(X,G,score_func); % initialize the score
G = PDAG2DAG(G);
                    ...
                    if(S(k)<2) % S indicate whether we need to check subset(k)
                        ...
                        if(V1)
                            ...
                            if(V2)
                                ...
                                [chscore,desc,record_local_score] = Insert_changed_score(X,G,i,j,sub{k},record_local_score,score_func);%calculate the changed score after Insert operator
        ...
        for j=1:N
            if((G(i,j)==-1 | G(i,j)==1)) % if Xi - Xj or Xi -> Xj
                ...
                for k=1:length(sub)
                    ...
                    if(V)
                        [chscore,desc,record_local_score] = Delete_changed_score(X,G,i,j,sub{k},record_local_score,score_func); %calculate the changed score after Insert operator
                        % desc{count} saves the corresponding (i,j,sub{k})
                        if(chscore<min_chscore)
                            min_chscore = chscore;
                            min_desc = desc;
