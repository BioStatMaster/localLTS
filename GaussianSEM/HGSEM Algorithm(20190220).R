...
###################
HGSEM_simulation_fun = function(seed, n_real, p_real, d = 1, alpha = 0.001, lambda = 1, path ){
  ...
  result_GSEM = HGSEM_Algorithm(data, alpha = alpha, sparsity_level = lambda, graph = graph)
  return(result_GSEM)
}

#### High Dimensional GSEM Learning using GLasso #################################
HGSEM_Algorithm = function(data, alpha = 0.001, sparsity_level = 1, graph = NULL){
  ...
  if(1 == 1){
    ...
    if( 1 == 1){
      ...
      #MB = huge(X, method = "mb")
      #MB = huge.select(MB)
      #MB = huge(X, method = "mb", lambda = sparsity_level*MB$opt.lambda) #half for small. 
      MB = huge(X, method = "mb", lambda = sparsity_level*sqrt(log(p)/n)) #half for small. 
      Omega = as.matrix( MB$beta[[1]] )
      Omega[Omega!=0] = 1
    }
    if( 1 == 2 ){
      library(huge)
      # DGSM applies graphical Lasso; otherwise applies Lasso
      GLasso = huge(X, method = "glasso", lambda = sparsity_level*sqrt(log(p)/n))
      Omega = GLasso$icov[[1]]
      # GLasso = huge.select(GLasso)
      # Omega = GLasso$opt.icov
      Omega[Omega!=0] = 1
    }
...
#  print(paste("Step (1) takes: ", proc.time()[3]- Runtime))
  ...
  while( length(RemNodes)>1 ){
    ...
    for(j in RemNodes){
      ...
      if( !identical( sort(ConditionSet[[j]]) , sort(k) ) && length(k) > 0 ){
        ConditionSet[[j]] = k
        score[j] = 1/( solve(cov(X[,c(j,k)]))[1,1] )
      }
    }
    #Odering Selection
  ...
  Ordering[p] = RemNodes  
  k = intersect(Ordering[1:(p-1)], which( Omega[,Ordering[p]] !=0 ) )
  if( !identical( sort(ConditionSet[[ Ordering[p]  ]]) , sort(k) ) && length(k) > 0 ){
    ConditionSet[[ Ordering[p] ]] = k
  }
  ...
  for(m in 2:p){
    j = Ordering[m]
    S = intersect( ConditionSet[[j]], Ordering[1:(m-1)] )
    if( !all( S ==0) ){
      if( length(S) > 1 ){
        A = solve( cov(X[,c(j,S)]) )
        r = -A[1,-1] /sqrt(A[1,1] * diag(A[-1,-1]) ) 
        parents = S[-abs(log( (1+r)/(1-r) )/2*sqrt(n-length(S)-3) ) < qnorm(alpha/2)]
        Estimated_G[j, parents] = 1
      }else if( length(S) == 1 ){
        parent_pvalue = ci.test(X[,j], X[,S], test = "cor")$p.value
  ...
  ####
  print(paste("HGSEM takes: ", Runtime))
  ####
  
  
  #A = Estimated_G + t(Estimated_G)
  #which( A ==2, arr.ind = T)
  ####
